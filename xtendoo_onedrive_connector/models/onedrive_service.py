...
class OneDriveService(models.AbstractModel):
    ...
    def download_file(self, onedrive_id, document_id=None):
        """
        Descarga un archivo de OneDrive y registra la acción en el chatter
        """
        token = self._get_token()
        headers = {'Authorization': f'Bearer {token}'}
        ...
        # Registrar la descarga en el chatter del documento
        try:
            ...
            if document and document.exists():
                ...
                _logger.info('Descarga registrada en chatter para documento: %s por usuario: %s',
                           document.name, self.env.user.name)
            else:
